...
const Blogs = () => {
    ...
    const [item, setItem] = useState('Copy')
    const [newLink, setNewLink] = useState('')
    const [originalLink, setOriginalLink] = useState('')
    const [display, setDisplay] = useState('none');

    
    // Shorten it click event
    const handleClick = () => {
        if (link === '') {
            setField('invalid')
            setAdd('add')
            ...
            .then((data) => {
                console.log(data);
                let originalLinkData = data.result.original_link
                let newLinkData = data.result.short_link3
                setOriginalLink(originalLinkData)
    ...
    const copyTextToClipboard = async () => {
        if ('clipboard' in navigator) {
            return await navigator.clipboard.writeText(newLink)
        }else{
    ...
    return ( 
        <div className="Blogs">
            <div className="inner-blogs">
                <div className="one">
                    <input type="text" className={field} value={link} onChange = {e => setLink(e.target.value)} placeholder="Shoten a link here..." />
                    <div className="shorten-it" onClick={handleClick}>
                        <p>Shorten it!</p>
                    </div>
                </div>
                    <span className={add}> <i>Please add a link</i> </span>
                <div className="storage" id={display}>
                    <div className="one">
                        <p>{ originalLink }</p>
                        <div className="new-link">
                            <p id="copyLink"> {newLink} </p>
                            <div className="copy" id={copy} onClick={handleCopyClick}>
                                <p>{item}</p>
                            </div>
                        </div>
                    </div>
